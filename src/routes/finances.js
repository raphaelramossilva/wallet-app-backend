  ...
  try {
    ...
    if (!title && title.length < 3) {
      return res.status(400).json({
        error: "Title is mandatory and should have more 3 characteres.",
      });
    }

    if (!date || date.length != 10) {
      return res.status(400).json({
        error: "Date is mandatory and should be in theformat yyyy-mm-dd.",
      });
    }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (financeItemQuery.rows[0].user_id !== userQuery.rows[0].id) {
      return res
        .status(401)
        .json({ error: "Finance row doesnt not belong to user" });
    }
